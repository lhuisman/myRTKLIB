    ...
    : QDialog(parent)
{
    ...
    connect(btnCancel, &QPushButton::clicked, this, &CodeOptDialog::reject);
    connect(btnOk, &QPushButton::clicked, this, &CodeOptDialog::btnOkClicked);
    connect(btnSetAll, &QPushButton::clicked, this, &CodeOptDialog::btnSetAllClicked);
}
//---------------------------------------------------------------------------
void CodeOptDialog::showEvent(QShowEvent *event)
{
    ...
    for (int i = 0; i < 7; i++) strncpy(mask[i], qPrintable(codeMask[i]), MAXCODE);
...
void CodeOptDialog::btnOkClicked()
{
    ...
    for (int i = 0; i < 7; i++) codeMask[i] = mask[i];
