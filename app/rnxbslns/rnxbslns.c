...
*
* LH
*           2020/12/09      modified for multiple baseline processing
*                           added options -rov -base -dtype 
*                                         -lstfile -rovfile -basefile
*-----------------------------------------------------------------------------*/
#include <stdarg.h>
#include "rtklib.h"
...
" -rov str  name of a rover station",
" -base str name of a base station",
" -dtype str name of a datasource (from lstfile)",
" -rovfile file  file with rover stations on each line",
" -basefile file file with base stations on each line",
" -lstfile file  file with download locations (see Download API for format"
};
/* show message --------------------------------------------------------------*/
...
static int dwnldopts(const char *file, char **stas, int *nstas)
{
    FILE *fp;
    char buff[2048],*p;
    int n=0,nsta=0,i,havetype,havesta;
    
    trace(3,"dwnldopts: file=%s\n",file);
    
    if (!(fp=fopen(file,"r"))) {
        trace(1,"dwnldopts: options file open error (%s)\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (havesta==0) {
            stas[nsta] = (char *)malloc(10 * sizeof(char));     
            strcpy(stas[nsta],p);
            nsta++;
        }
...
int main(int argc, char **argv)
{
    ...
    /* delcarations for automatic downloads */
    int nurl;
    int nmax=10;
    int seqnos=0;
    int seqnoe=0;
    const char dira[1024] = "";
    const char usr[1024]="anonymous";
    const char pwd[1024]="";
    const char proxy[1024]="";
    int opts=DLOPT_KEEPCMP;
    char *msg;
    url_t urls[4];
    int ti=86400;
    char localFiles[MAXFILE][1024];
    char local[1024];
    /* Declarations for rover and base */
    char rovstr[1024]="";
    int nrov=0;
    char *bases[MAXRCV];
    char basestr[1024]="";
    ...
    /* Command line overrides of options */
    for (i=1,n=0;i<argc;i++) {
        ...
        /* new options for rtkdefo */
        if (!strcmp(argv[i],"-basefile")&&i+1<argc) {
            strcpy(dwnldopt.basefile,argv[++i]);
            dwnldopts(dwnldopt.basefile,&dwnldopt.bases[0],&nbase );
        }
        if (!strcmp(argv[i],"-rovfile")&&i+1<argc) {
            strcpy(dwnldopt.rovfile,argv[++i]);
            dwnldopts(dwnldopt.rovfile,&dwnldopt.rovers[0],&nrov);
        }
        if ((!strcmp(argv[i],"-lstfile") || !strcmp(argv[i],"-lst"))&&i+1<argc) {
            strcpy(dwnldopt.lstfile,argv[++i]);
        }
        else if (!strcmp(argv[i],"-base")&&i+1<argc) {
            dwnldopt.bases[nbase] = (char *)malloc(10 * sizeof(char));     
            strcpy(dwnldopt.bases[nbase],argv[++i]);
            bases[nbase] = (char *)malloc(10 * sizeof(char));     
            bases[nbase]=argv[i];
            nbase++;
            
        }
        else if (!strcmp(argv[i],"-dtype")&&i+1<argc) {
            dwnldopt.dtypes[ntype] = (char *)malloc(10 * sizeof(char));     
            dwnldopt.dtypes[ntype]=argv[++i];
            ntype++;
        }
        else if (!strcmp(argv[i],"-rov")&&i+1<argc) {
            dwnldopt.rovers[nrov] = (char *)malloc(10 * sizeof(char));     
            dwnldopt.rovers[nrov]=argv[++i];
            nrov++;
        }
    ...
    if (nrov<=0) {
        showmsg("error : no rover station defined");
        return -2;
    }
    
    /* Create string with rovers and bases */
    for (i=0;i<nrov;i++) {
            strcat(rovstr,dwnldopt.rovers[i]);
            strcat(rovstr," ");
    }
    for (i=0;i<nbase;i++) {
            strcat(basestr,dwnldopt.bases[i]);
            strcat(basestr," ");
    }    
    ...
    nurl=dl_readurls(&dwnldopt.lstfile[0], dwnldopt.dtypes, ntype, &urls[0], nmax);
    for (i=0;i<nurl;i++) {
        remot2local(urls[i].path, urls[i].dir, local);
        strcpy(localFiles[ninfile],local);
        infile[ninfile]=localFiles[ninfile];
        ninfile++;
    }

    dl_exec(ts, te, ti, seqnos, seqnoe,
        urls, nurl, &dwnldopt.rovers[0], nrov,
        &dira[0], &usr[0], &pwd[0],
        &proxy[0], opts, msg, NULL);

    dl_exec(ts, te, ti, seqnos, seqnoe,
        urls, nurl, &dwnldopt.bases[0], nbase,
        &dira[0], &usr[0], &pwd[0],
        &proxy[0], opts, msg, NULL);
    ...
    /* Start processing */
    ret=postpos(ts,te,tint,0.0,&prcopt,&solopt,&filopt,infile,ninfile,outfile,rovstr,basestr);
