...
static double varerr(int sat, int sys, double el, double snr_rover, double snr_base, 
                     double bl, double dt, int f, const prcopt_t *opt)
{
        ...
        for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
            ...
            for (j=0;j<ns;j++) {
                ...
                Ri[nv] = varerr(sat[i], sysi, azel[1+iu[i]*2], 
                                0.25 * rtk->ssat[sat[i]-1].snr_rover[ff],
                                0.25 * rtk->ssat[sat[i]-1].snr_base[ff],
                                bl, dt, f, opt);
                Rj[nv] = varerr(sat[j], sysj, azel[1+iu[j]*2], 
                                0.25 * rtk->ssat[sat[j]-1].snr_rover[ff],
                                0.25 * rtk->ssat[sat[j]-1].snr_base[ff],
                                bl, dt, f, opt);
